...
const CardActions: React.FC<Props> = ({ frozen, onFreeze }) => {
  ...
  return (
    <div className="flex justify-between items-start py-5 px-6 bg-[#EDF3FF] rounded-b-[12px] mt-[-8px]">
      {actions.map((action, i) => (
        <button
          key={i}
          onClick={action.onClick}
          className="flex flex-col items-center gap-2 text-[13px] text-[#0C365A] hover:opacity-80 transition-opacity w-[64px]"
        >
          <div className="w-8 h-8 rounded-full bg-[#325BAF] flex items-center justify-center">
            <img src={action.icon} className="w-6 h-6" alt={action.label} />
          </div>
